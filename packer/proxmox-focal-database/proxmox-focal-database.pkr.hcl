...
source "proxmox-iso" "proxmox-focal-database" {
...
build {
  sources = ["source.proxmox-iso.proxmox-focal-database"]
  ...
  provisioner "shell" {
    execute_command = "echo 'vagrant' | {{ .Vars }} sudo -E -S sh '{{ .Path }}'"
    scripts          = ["../scripts/proxmox/focal-database/post_install_prxmx_ubuntu_firewall-additions.sh",
                        "../scripts/proxmox/focal-database/post_install_prxmx_ubuntu_install-database-software.sh"
    ]
  }
