...
source "proxmox-iso" "proxmox-focal-ubuntu-docker" {
...
build {
  sources = ["source.proxmox-iso.proxmox-focal-ubuntu-docker"]
  ...
  ########################################################################################################################
    ...
    provisioner "shell" {
      execute_command = "echo 'vagrant' | {{ .Vars }} sudo -E -S sh '{{ .Path }}'"
      scripts          = ["../scripts/proxmox/focal-docker/post_install_prxmx_ubuntu_install-docker.sh"]
    }
