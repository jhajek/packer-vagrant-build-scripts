...
source "proxmox-iso" "proxmox-rocky-podman" {
...
build {
  description = "Build base RockyLinux 8.5 and and podman"

  sources = ["source.proxmox-iso.proxmox-rocky-podman"]
  ...
  ########################################################################################################################
    ...
    provisioner "shell" {
      execute_command = "echo 'vagrant' | {{ .Vars }} sudo -E -S sh '{{ .Path }}'"
      scripts          = ["../scripts/proxmox/rocky-podman/post_install_prxmx_install-podman.sh"]
    }
